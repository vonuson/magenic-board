...
export class CardDetailComponent implements OnInit {
  ...
  private onSubmit(form): void {
    if (form.value.cardEntry) {
      ...
      const newCard = new Card(this.cardList.boardId, this.cardList.id, form.value.cardEntry, nextOrder);
    ...
    this.CardPopupMoveComponentRef.afterClosed().subscribe(data => {
      ...
      if (data && !(data.listId === card.cardListId && data.newOrder === prevOrder)) {
        ...
        if (data.listId === card.cardListId) {
          const filteredCards = this.cards.filter(crd => (crd.order >= data.newOrder) || (crd.order >= prevOrder));

          for (let i = 0; i < filteredCards.length; i++) {
            const itrCard = filteredCards[i];
          ...
          this.cardDetailService.getCardsById(String(this.cardList.boardId), data.listId)
            .pipe(
              flatMap((list: ICard[]) => {
                return of(list);
              })
              ...
              const filteredDestinationCards = res.filter(crd => crd.order >= data.newOrder);
              for (let i = 0; i < filteredDestinationCards.length; i++) {
                filteredDestinationCards[i].order += 1;
              }

              const filteredSourceCards = this.cards.filter(crd => crd.order >= prevOrder);
              for (let i = 0; i < filteredSourceCards.length; i++) {
                const itrCard = filteredSourceCards[i];
